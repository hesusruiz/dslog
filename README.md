...
This document describes a standard timestamping service ([**RFC 3161 Time-Stamp Protocol (TSP)**](https://www.rfc-editor.org/rfc/rfc3161)) combined with a tamper-evident log based on the modern [**Tiled Transparency Logs**](https://github.com/C2SP/C2SP/blob/main/tlog-tiles.md) specification, and a [**public-permissioned blockchain**](https://www.linkedin.com/pulse/public-permissioned-blockchains-common-pool-resources-jesus-ruiz/) (ISBE Blockchain, based on BESU) to create a fast, verifiable, Byzantine-resistant and globally consistent timestamping service. This hybrid approach addresses critical limitations of traditional timestamping, providing strong guarantees of data existence, inclusion, and immutability without requiring blind trust in any single operator.

The system enables massive scalabilitity with the ability to operate many different uncoordinated logs and where log operators can be untrusted entities. The system preserves the most important properties of a standard blockchain implementation, but without requiring all transactions to be written to the blockchain.
...
* **Preserving Trust Properties:** This batching mechanism does not compromise trust. The Merkle tree structure ensures that the STH is a commitment to _every single entry_ in the batch and to all previous entries in the log. Any attempt to tamper with a past entry would change the STH, which would be detected during verification against the immutable anchor published on the blockchain. Global consistency and protection against split-view attacks are guaranteed by the blockchain-anchored checkpoints.
...
## The timestamping service implementation
...
## Deployment Considerations (this section needs more work)
